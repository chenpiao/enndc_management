...
class ListManager(models.Manager):
    ...
    def vms_all(self):
        return self.filter(delete_time=None)


class List(models.Model):
    instance_uuid = models.CharField(verbose_name='UUID', max_length=255, null=True, blank=True, unique=True)
    list_name = models.CharField(verbose_name='清单主机名', max_length=255, null=True, db_index=True)
    hostname = models.CharField(verbose_name='主机名', max_length=255, null=True)
    ip = models.GenericIPAddressField(verbose_name='IP地址', null=True, db_index=True)
    ...
    total_hard_disk = models.IntegerField(verbose_name='总硬盘容量', blank=True, null=True, db_index=True)
    cpu = models.IntegerField(verbose_name='CPU核心数量', blank=True, null=True, db_index=True)
    mem = models.IntegerField(verbose_name='内存', blank=True, null=True, db_index=True)
    ...
    # 管理信息
    app_name = models.CharField(verbose_name='应用名称', max_length=100, blank=True, null=True, db_index=True)
